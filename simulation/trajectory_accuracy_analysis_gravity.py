...
class DroneSimulator:
    ...
    def update(self, target_position, target_velocity, target_acceleration, dt):
        ...
        """
        # Simulate a noisy position measurement, which is what the controller sees
        position_measurement_noise = np.random.normal(
            0, self.disturbance_levels['position'], size=3)
        measured_position = self.position + position_measurement_noise

        # Calculate position error based on the noisy measurement
        ...
        # dimensionally incorrect (adding velocity to force) and is removed.
        # force_cmd += 1.0 * (target_velocity - self.velocity)
        
        # This is a random force disturbance (e.g., wind gusts)
        force_disturbance = np.random.normal(
            0, self.disturbance_levels['velocity'], size=3)
        
        # Update acceleration based on F=ma -> a = F/m
        total_force = force_cmd + force_disturbance
        ...
        if accel_norm > max_accel:
            self.acceleration = self.acceleration * (max_accel / accel_norm)
            
        # Update the TRUE physical position using the velocity from the START of the timestep
        self.position += self.velocity * dt
        
        # Now, update the velocity for the NEXT timestep
...
class TrajectoryAccuracySimulation:
    ...
    def __init__(self, waypoints, segment_times=None, rtk_enabled=True, wind_strength=0.0, sim_options=None, pid_gains_override=None):
        """
        ...
        Args:
            ...
            pid_gains_override: Dictionary to override default PID gains.
        """
        ...
        if pid_gains_override:
            pid_gains = pid_gains_override
        else:
             # Final PID gains tuned for a critically damped response.
        ...
        if segment_times is None:
            # The previous speed was too high, creating a dynamically infeasible trajectory.
            # Reducing the speed gives the drone more time, requiring less acceleration.
            desired_speed = 5.0 if rtk_enabled else 4.0  # m/s
            segment_times = []
    ...
    def run_simulation(self, dt=0.02, debug=False):
        """
        Run the trajectory tracking simulation.
        
        Args:
            dt: Time step in seconds (default: 0.02s = 50Hz)
            debug: If True, print detailed state information.
        """
        ...
        while t <= self.trajectory.total_time:
            ...
            if debug and step_counter % 10 == 0: # Print every 25 steps (0.5s)
                print(f"--- t={t:.2f}s ---")
                print(f"  Target Pos:  {planned_position[0]:>8.3f}, {planned_position[1]:>8.3f}, {planned_position[2]:>8.3f}")
                print(f"  Actual Pos:  {actual_position[0]:>8.3f}, {actual_position[1]:>8.3f}, {actual_position[2]:>8.3f}")
                # Access internal state of the simulator for debugging
                p_err = self.drone_sim.prev_error
                force = self.drone_sim.last_force_cmd
                accel = self.drone_sim.acceleration
                vel = self.drone_sim.velocity
                print(f"  Pos Error:   {p_err[0]:>8.3f}, {p_err[1]:>8.3f}, {p_err[2]:>8.3f}")
                print(f"  Force Cmd:   {force[0]:>8.3f}, {force[1]:>8.3f}, {force[2]:>8.3f}")
                print(f"  Acceleration:{accel[0]:>8.3f}, {accel[1]:>8.3f}, {accel[2]:>8.3f}")
                print(f"  Velocity:    {vel[0]:>8.3f}, {vel[1]:>8.3f}, {vel[2]:>8.3f}")
    ...
    def create_video(self, filename='trajectory_video.mp4', fps=30):
        ...
        def update(frame):
            ...
            # Update drone position
            drone_marker._offsets3d = ([self.results['actual_position'][frame, 0]], 
                                       [self.results['actual_position'][frame, 1]], 
                                       [self.results['actual_position'][frame, 2]])
            
            # Update error line (connection from drone to planned position)
            error_line.set_data([self.results['actual_position'][frame, 0], 
                                 self.results['planned_position'][frame, 0]], 
                                [self.results['actual_position'][frame, 1], 
                                 self.results['planned_position'][frame, 1]])
            error_line.set_3d_properties([self.results['actual_position'][frame, 2], 
                                           self.results['planned_position'][frame, 2]])
...
def auto_tune_pid(waypoints, rtk_enabled, wind_strength, sim_options, n_iterations=100):
    """
    Performs a random search to find optimal PID gains.

    Args:
        waypoints: List of waypoints for the simulation.
        rtk_enabled: Whether RTK is enabled.
        wind_strength: Wind strength for the simulation.
        sim_options: Dictionary of simulation options.
        n_iterations: Number of random parameter sets to test.
    """
    print(f"Starting automated PID tuning with {n_iterations} iterations...")
    ...
    for i in range(n_iterations):
        # Randomly sample gains from the search space
        kp = np.random.uniform(*search_space['kp'])
        ki = np.random.uniform(*search_space['ki'])
        kd = np.random.uniform(*search_space['kd'])
        ...
        sim = TrajectoryAccuracySimulation(
            waypoints, 
            rtk_enabled=rtk_enabled, 
            wind_strength=wind_strength, 
            sim_options=sim_options,
            pid_gains_override=current_pid_gains
        )

        sim.run_simulation(dt=sim_options.get('dt', 0.02))
        analysis = sim.analyze_results()
        current_rmse = analysis['rmse_position']

        progress = (i + 1) / n_iterations
        bar_length = 30
        block = int(round(bar_length * progress))
        progress_text = f"\rIter {i+1}/{n_iterations} [{'#' * block + '-' * (bar_length - block)}] Kp={kp:.2f}, Ki={ki:.2f}, Kd={kd:.2f} -> RMSE={current_rmse:.4f}m"
        sys.stdout.write(progress_text)
        sys.stdout.flush()

        if current_rmse < best_rmse:
            best_rmse = current_rmse
            best_gains = current_pid_gains
            sys.stdout.write('\n')
            print(f"  -> New best RMSE found: {best_rmse:.4f}m with Kp={kp:.2f}, Ki={ki:.2f}, Kd={kd:.2f}")

    print("\n\n--- PID Tuning Complete ---")
    if best_gains:
        print(f"Best RMSE: {best_rmse:.4f} m")
        print("Best PID Gains:")
        print(f"  Kp: {best_gains['kp'][0]:.4f}")
        print(f"  Ki: {best_gains['ki'][0]:.4f}")
        print(f"  Kd: {best_gains['kd'][0]:.4f}")
    else:
        print("Tuning did not find any valid results.")
        
    return best_gains, best_rmse
...
def main():
    ...
    parser.add_argument('--auto-tune', action='store_true',
                        help='Automatically search for optimal PID gains using random search.')
    parser.add_argument('--kp', type=float, help='Override Kp gain for a single run.')
    parser.add_argument('--ki', type=float, help='Override Ki gain for a single run.')
    parser.add_argument('--kd', type=float, help='Override Kd gain for a single run.')
    ...
    parser.add_argument('--debug', action='store_true',
                        help='Print detailed state information during simulation for debugging')
    ...
    if args.auto_tune:
        auto_tune_pid(waypoints, args.rtk, args.wind, sim_options, n_iterations=100)
        return

    pid_gains_override = None
    if args.kp is not None and args.ki is not None and args.kd is not None:
        pid_gains_override = {
            'kp': np.array([args.kp, args.kp, args.kp]),
            'ki': np.array([args.ki, args.ki, args.ki]),
            'kd': np.array([args.kd, args.kd, args.kd])
        }
    ...
    if args.compare:
        ...
        sim.run_simulation(dt=sim_options['dt'], debug=args.debug)
